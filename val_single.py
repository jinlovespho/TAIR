...
def main(args):
    ...
    lq_imgs_path = [cfg.dataset.lq_img_path]
    ...
    for val_batch_idx, lq_img_path in enumerate(tqdm(lq_imgs_path, desc='val', total=len(lq_imgs_path))):
        
        # gt_id = gt_img_path.split('/')[-1].split('.')[0]
        lq_id = lq_img_path.split('.')[0]
        ...
        with torch.no_grad():
            ...
            #             })
                ...
                wandb.log({ 
                        #    f'sampling_val_FINAL_VIS/{gt_id}_val_gt': wandb.Image((val_gt + 1) / 2, caption=f'gt_img'),
                ...
                wandb.log({f'sampling_val_FINAL_VIS/{gt_id}_val_all': wandb.Image(torch.concat([val_lq, val_clean, torch.clamp((pure_cldm.vae_decode(val_z) + 1) / 2, 0, 1)], dim=2), caption='lq_clean_sample')})
